    ...
    <div class="share-content clearfix">
      ...
      <div class="discount-item top-line" v-if="activityList.length>0">
        <div class="row" v-for="(item,index) in activityList" v-show="index==0 || item.flag==true">
          ...
          <div @click="displayActivity()" v-if="index==0">
            <span>{{activityList.length}}个活动</span>
            <span class="triangle" :class="{'display':display == true}" v-if="activityNum>1"></span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    goStore() {
      ...
      let url = "";
      if (this.$sign == "wx") {
        url = this.baseUrl + "/grantShareShop?shopId=" + storeId;
      } else {
        url = this.baseUrl + "/scanCode?shopId=" + storeId;
      }
