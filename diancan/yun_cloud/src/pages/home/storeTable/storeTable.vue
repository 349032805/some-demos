<template>
  <div class="store_table" ref="store_table">
    ...
    <div class="table_content" v-if="type==1" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="1">
      <div>
        <store-door v-for='(item,index) in storeDoor' :info="item" :key='index'></store-door>
...
import DoneStore from './doneStore/doneStore.vue'
import NoStore from './noStore/noStore.vue'
export default {
  name: 'storeTable',
  created() {
    //从搜索入口进来的从新给vux
  ...
  computed: {
     ...mapState({
      LOCATION: state => state.homeModel.LOCATION
    })
  },
  watch: {
    LOCATION: {
