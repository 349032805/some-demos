<template>
  <div class="my-collection" v-infinite-scroll="_loadList" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
    <!-- <div class="item">
          ...
          <div slot='content'>
            <div class="item">
              <div class="store-logo">
                ...
                <img src="../../assets/store-logo.png" v-if="!item.fsLogo">
              </div>
              <div class="store">
                <p class="store-name overflow">{{item.fsShopName}}</p>
                <p class="business-distance">
                  <span style="margin-right: .2rem;">{{item.fsCuisineSeries}}</span>
...
export default {
  ...
  methods: {
    ...
    goDetail(storeId) {
      this.$router.push({ path: `/menus/${storeId}/${localStorage.token}`, query: { sign: this.$sign } });
    },
    _loadList() {
      let _this = this;
      this.loading = true;
