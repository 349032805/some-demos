...
import kitt.core.domain.RoleMenu;
import kitt.core.domain.UserRole;
...
public interface MenuMapper {
    ...
    @Select("select * from menus where parentid=#{parentid} order by sequence, id asc")
    ...
    public default Object saveMenu(Menu menu){
        ...
        if(menu.getId() != null){
            if(getMenuById(menu.getId()) == null){
                ...
                success = updateMenu(menu) == 1 ? true : false;
            }
        } else {
            success = addMenu(menu) == 1 ? true : false;
        }
