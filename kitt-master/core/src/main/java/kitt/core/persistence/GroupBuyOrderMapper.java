...
import kitt.core.domain.GroupBuyOrder;
...
public interface GroupBuyOrderMapper {
    ...
    @Select("select * from groupbuyorder where qualificationcode=#{qualificationcode} and groupbuysupplycode=#{groupbuysupplycode} and isdelete=false")
    public GroupBuyOrder getOrderByQualifyCode(@Param("qualificationcode")String qualificationcode,@Param("groupbuysupplycode")String groupbuysupplycode);
    ...
    @Select("select * from groupbuyorder where groupbuysupplyid=#{groupbuysupplyid} and isdelete=true order by createtime DESC")
    public List<GroupBuyOrder> getDeleteOrdersBySupplyId(@Param("groupbuysupplyid")int groupbuysupplyid);
    ...
    @Update("update groupbuyorder set volume=#{volume} where id=#{id} and isdelete=false")
    public void updateVolumeById(@Param("volume")int volume,@Param("id")int id);
    ...
    public int deleteOrder(@Param("groupbuysupplycode")String groupbuysupplycode,@Param("qualificationcode")String qualificationcode);

    @Select("select * from groupbuyorder  where groupbuysupplycode=#{groupbuysupplycode} and qualificationcode=#{qualificationcode} and status='ORDER_CREATE' and isdelete=false")
    public GroupBuyOrder getOrderbycode(@Param("groupbuysupplycode")String groupbuysupplycode,@Param("qualificationcode")String qualificationcode);
    ...
    @Insert("insert into groupbuyorder(groupbuysupplyid, userid, qualificationcode,groupbuyordercode,status," +
            "volume,createtime,isdelete,contractverify,performancecode, groupbuysupplycode, clienttype) values(#{groupbuysupplyid}," +
            "#{userid}, #{qualificationcode}, dateseq_next_value('TGDD'),#{status},#{volume},#{createtime},false,#{contractverify}," +
            "dateseq_next_value('LYS'), #{groupbuysupplycode}, #{clienttype})")
    @Options(useGeneratedKeys=true)
    public void addGroupBuyOrder(GroupBuyOrder groupBuyOrder);

    @Delete("delete from `groupbuyorder` where status='ORDER_CREATE'")
