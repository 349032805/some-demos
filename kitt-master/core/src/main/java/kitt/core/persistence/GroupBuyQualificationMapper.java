...
import kitt.core.domain.GroupBuyQualification;
...
public interface GroupBuyQualificationMapper {
    ...
    @Select("select count(id) from groupbuyqualification where userid =#{userid} and isdelete=false and status!='QUALIFY_START' ")
    public int countGroupBuyQualifyBuStatusId(@Param("userid")int userid);

    @Insert("insert into groupbuyqualification(comment,userid, qualificationcode, status, groupbuyordercode,createtime,isdelete, marginscode, clienttype,confirmtime) values(#{comment},#{userid}, dateseq_next_value('TGZZ'), #{status}, #{groupbuyordercode}, now(), false, dateseq_next_value('TGHT'), #{clienttype},#{confirmtime})")
    @Options(useGeneratedKeys=true)
    public void addGroupBuyQualify(GroupBuyQualification groupBuyQualification);

    @Update("update groupbuyqualification set margins=#{margins}, marginscode=#{marginscode}, photopath=#{photopath}, comment=#{comment}, qualificationcode=#{qualificationcode}, status=#{status}, createtime=#{createtime}, groupbuyordercode=#{groupbuyordercode},confirmtime=#{confirmtime} where id=#{id}")
    ...
    public default Pager<GroupBuyQualification> pageAllGroupBuyQualifyByStatusId(String status,int page, int pagesize){
        return Pager.config(this.countAllQualifyByStatusId(status), (int limit, int offset) -> this.listAllGroupBuyQualifyByStatusId(status,limit, offset)).page(page, pagesize);
    }
    @Select("select count(*) from groupbuyqualification where status=#{status} and isdelete=false")
    public int countAllQualifyByStatusId(@Param("status")String status);
    @Select("select * from groupbuyqualification where status=#{status} and isdelete=0 limit #{limit} offset #{offset}")
    public List<GroupBuyQualification> listAllGroupBuyQualifyByStatusId(@Param("status")String status, @Param("limit") int limit, @Param("offset") int offset);


    public default Pager<GroupBuyQualification> pageAllGroupBuyQualifyByStatus(String status,int page, int pagesize){
        return Pager.config(this.countAllQualifyByStatus(status), (int limit, int offset) -> this.listAllGroupBuyQualifyByStatus(status,limit, offset)).page(page, pagesize);
    }
    @Select("select count(*) from groupbuyqualification where status=#{status}")
    public int countAllQualifyByStatus(@Param("status")String status);
    @Select("select * from groupbuyqualification where status=#{status} limit #{limit} offset #{offset}")
    public List<GroupBuyQualification> listAllGroupBuyQualifyByStatus(@Param("status")String status, @Param("limit") int limit, @Param("offset") int offset);
